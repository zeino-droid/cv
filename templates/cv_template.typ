...
#let leading-val        = float(sys.inputs.at("leading",        default: "0.50"))  * 1em
#let section-gap-val    = float(sys.inputs.at("section-gap",    default: "5.0"))   * 1pt
...
#let render-project(proj, index) = {
  block(spacing: 2.0pt, breakable: false)[
    ...
    #grid(
      ...
      gutter: 4pt,
      text(size: 7.5pt, fill: secondary, "[" + str(index + 1) + "]"),
      text(size: 9pt, weight: "semibold", fill: primary)[#proj.name],
      box(
        fill: divider-color,
        inset: (x: 4pt, y: 1.5pt),
        radius: 2pt,
        text(size: 6.5pt, fill: secondary, weight: "bold")[PROJET]
      )
    )
    #v(-0.15em)
    // Description plus compacte
    #text(size: 8.2pt, style: "italic", fill: secondary)[#proj.description]
    #v(-0.25em)
    // Keywords sur même ligne, plus serré
    #text(size: 7.5pt, fill: secondary.lighten(20%))[#proj.keywords]
  ]
        ...
        if cv_data.keys().contains("education") {
            for edu in cv_data.education {
              ...
              if edu.keys().contains("modules") and edu.modules != () and edu.modules != [] {
                  ...
                  if all_mods.len() > 0 {
                    v(-0.2em)
                    set text(size: 7.5pt + font-size-delta, fill: secondary.lighten(10%))
